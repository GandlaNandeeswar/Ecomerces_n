...
async function requireLogin(redirectTo = "/auth/login/") {
  if (!isLoggedIn()) {
...
async function requireAdmin(redirectTo = "/admin-portal/login/") {
...
async function apiFetch(path, options = {}) {
  ...
  if (res.status === 401 && !isAuthEndpoint) {
    const isAdminArea = window.location.pathname.startsWith("/admin-portal");
    window.location.href = isAdminArea ? "/admin-portal/login/" : "/auth/login/";
